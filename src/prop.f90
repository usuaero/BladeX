...
!    do i=1,10
    write(*,*)
    write(*,*) '  RPM                       Thrust [N]'
    do while (abs(t%Thrust-thrust)/thrust > thrust_error)
        call prop_solve(t,0)
        write(*,*) t%rpm,t%Thrust
        C0 = t%Thrust/(t%rpm)**2 !assumes quadratic
...
!-----------------------------------------------------------------------------------------------------------
subroutine prop_data_dump(t)
    type(prop_t) :: t
    character(100) :: filename
    integer :: isec,ierror
    type(section_t),pointer :: si
    120 Format(A15, 10ES25.13)

    filename = trim(adjustl(t%master_filename))//'_dump.txt'
    open(unit = 10, File = filename, action = 'write', iostat = ierror)
    write(10,*) ' PropName           ControlPoint(x)          ControlPoint(y)          ControlPoint(z)          &
           &Chord                    alpha(deg)               CL                       CD                       &
           &Cm                       area'
    do isec=1,t%nSec
        si => t%sec(isec)
        write(10,120) t%name,si%PC(:),si%chord_c,si%alpha*180.0/pi,sec_CL(si),sec_CD(si),sec_Cm(si),si%ds
    end do
    close(10)
end subroutine prop_data_dump
...
span = 0.0
    ...
    do isec=1,t%nSec
        ...
        t%sec(isec)%beta = atan(lambda/(2.0*pi*rc)) !aerodynamic pitch angle
        my_twist = t%sec(isec)%beta + aL0 !geometric pitch angle
        t%sec(isec)%aL0 = aL0
        ...
        t%sec(isec)%dihedral1 = 0.0
        t%sec(isec)%dihedral2 = 0.0
        ...
        if(t%f_pitch .ne. 'none') then !fix this. Need to update t%sec(isec)%beta for twist file input
